// netlify/functions/load-agreement.js
// Loads agreement data from a ClickUp task by ID for the signing page
//
...
exports.handler = async (event) => {
  ...
  try {
      ...
      switch (f.type) {
        ...
        case 'labels':
          if (Array.isArray(f.value)) {
            return f.value.map(v => v.label || v.name || v).filter(Boolean);
          }
          return [];
        default:
          return f.value;
    ...
    // Check if already signed (status not 'to do')
    const status = task.status ? task.status.status.toLowerCase() : 'to do';
    const isSigned = status !== 'to do';
