...
async function sendClientEmail(data) {
  ...
  try {
    ...
    await fetch('https://gmail.googleapis.com/gmail/v1/users/me/messages/send', { method:'POST', headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`}, body:JSON.stringify({raw}) });
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (token) {
      // UPDATE existing task (client signed from portal/token link)
      const updatePayload = {
        name: taskName,
        description,
        status: 'complete',
      ...
      if (!res.ok) {
        ...
        return { statusCode: 502, headers, body: JSON.stringify({ message: 'Failed to update agreement in ClickUp' }) };
      }

      task = await res.json();
    } else {
      // CREATE new task (no existing task, direct form submission)
      const createPayload = {
        name: taskName,
        description,
        status: 'complete',
      ...
      if (!res.ok) {
        ...
        return { statusCode: 502, headers, body: JSON.stringify({ message: 'Failed to create agreement in ClickUp' }) };
      }
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ message: 'Agreement submitted', taskId: task.id, taskUrl: task.url }),
    };
