...
exports.handler = async (event) => {
  ...
  const taskPayload = { name:taskName, description, status:'to do', custom_fields:customFields };
  try {
    const res = await fetch(`https://api.clickup.com/api/v2/list/${LIST_ID}/task`, { method:'POST', headers:{'Content-Type':'application/json','Authorization':CLICKUP_API_TOKEN}, body:JSON.stringify(taskPayload) });
    if (!res.ok) { const err = await res.text(); console.error('ClickUp error:', res.status, err); return { statusCode:502, headers, body:JSON.stringify({message:'Failed to create agreement in ClickUp'}) }; }
    const task = await res.json();
    if (data.signatureImage) { try { const b64 = data.signatureImage.replace(/^data:image\/png;base64,/,''); const buf = Buffer.from(b64,'base64'); const boundary = '----Boundary'+Date.now().toString(36); const fn = `signature-${data.clientName.replace(/[^a-zA-Z0-9]/g,'-')}.png`; const start = Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="attachment"; filename="${fn}"\r\nContent-Type: image/png\r\n\r\n`); const end = Buffer.from(`\r\n--${boundary}--\r\n`); await fetch(`https://api.clickup.com/api/v2/task/${task.id}/attachment`, { method:'POST', headers:{'Authorization':CLICKUP_API_TOKEN,'Content-Type':`multipart/form-data; boundary=${boundary}`}, body:Buffer.concat([start,buf,end]) }); } catch(e) { console.error('Sig upload failed:', e.message); } }
  ...
  } catch(err) { console.error('Error:', err); return { statusCode:500, headers, body:JSON.stringify({message:'Internal server error'}) }; }
};